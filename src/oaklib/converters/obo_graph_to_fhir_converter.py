...
class OboGraphToFHIRConverter(DataModelConverter):
    ...
    # TODO: `retention` is temporary. I think we likely have to give this a different name/functionality, as well as put
    #  it in the CLI and possibly elsewhere. I'm also doing this a pure way, passing it to dump and then further down,
    #  rather than an instance property. - Joe 2022/12/01
    def dump(self, source: GraphDocument, target: str = None, retention: str = ['maximal', 'minimal'][0]) -> None:
        """
        ...
        :return:
        """
        cs = self.convert(source, retention=retention)
    ...
    def convert(
        self, source: GraphDocument, target: CodeSystem = None, retention: str = ['maximal', 'minimal'][0]
    ) -> CodeSystem:
        """
        ...
        for g in source.graphs:
            self._convert_graph(g, target=target, retention=retention)
        return target
    ...
    def _convert_graph(
        self, source: Graph, target: CodeSystem, retention: str = ['maximal', 'minimal'][0]
    ) -> CodeSystem:
        target.id = source.id
        ...
        for n in source.nodes:
            ...
            self._convert_node(n, index=edges_by_subject, target=target, retention=retention)
        return target

    def _convert_node(
        self, source: Node, index: Dict[CURIE, List[Edge]], target: CodeSystem,
        retention: str = ['maximal', 'minimal'][0]
    ) -> Concept:
        ...
        for e in index.get(source.id, []):
            obj = self.code(e.obj)
            if retention == 'maximal' or e.pred in DIRECT_PREDICATE_MAP:
